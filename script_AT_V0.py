  ...
  try:
    ...
    with rasterio.open(DEM) as src:
        ...
        for t_idx, tile_geom in enumerate(tqdm(tiles, desc="Procesando tiles")):
            ...
            del out_image, out_transform, points_in_tile, stats
            gc.collect()
    ...
    result = gpd.GeoDataFrame.from_features(result_chunks, crs=points_buffered.crs)
    
    print("GeoDataframe Stats 1:",result.shape)
    ...
    umbral = 2000
    result_filtrado = result[result['dem_median'] > umbral]
    ...
    gdf_final = pd.concat([result_filtrado.reset_index(drop=True), stats_df], axis=1)
    gdf_final = gpd.GeoDataFrame(gdf_final, geometry='geometry', crs=result_filtrado.crs)
...
gdf_final=filt_csv('D:/MesProgrammes/MCD14ML/fire_archive_M-C61_706555.csv',
