...
def download_tile(lat_min, lat_max, lon_min, lon_max, tile_name, output_dir, scale=30):
    """Descarga una tile individual"""
    
    region = ee.Geometry.Rectangle([lon_min, lat_min, lon_max, lat_max])
    tile_dem = dem.clip(region)
...
output_dir = './copernicus_dem_andes/'
...
print("="*70)
print("DESCARGA COPERNICUS DEM - REGIÓN ANDES")
...
for i, (region_name, bounds) in enumerate(regions.items(), 1):
    ...
    downloaded, failed = download_region_with_fixed_tiles(
        ...
        bounds, 
        output_dir, 
        tile_size=0.5,  # 1 grado = ~15-40 MB
        scale=30
    )
