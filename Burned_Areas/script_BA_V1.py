...
def load_worldcover(wc_path, roi_geom_list, target_shape, target_transform):
    """Ahora solo se llama UNA vez desde run_pipeline."""
    with timer("load_worldcover: lectura y recorte"):
        with rasterio.open(wc_path) as src:
            wc_data, wc_transform = rasterio.mask.mask(
...
def run_pipeline():
    ...
    with timer("carga WorldCover"):
        wc_data, wc_transform = load_worldcover(
            data_dir / 'mosaico_andes_WC.tif', roi_geom_list,
            target_shape=None, target_transform=None   # ya no necesita estos params
        )
    ...
    with timer("escritura GeoPackage"):
        ...
        gdf_final.to_file(test_dir / "burned_areas_final_3.gpkg", driver="GPKG")
