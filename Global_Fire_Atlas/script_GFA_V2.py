...
"""
Version 2.0.0  (prefijo de salida "GFA_V2")
@author: jvilla
...
COUNTRY_FILTER = "Peru"
CENTROID_CRS = "EPSG:3857"               # coherente con area_ha
...
def run_worker(worker_id, n_workers):
    ...
    print(f"\n{'='*58}\n  WORKER {worker_id}/{n_workers} — GFA_V2")
    ...
    gpkg_path = test_dir / f"GFA_V2_{run_tag}_w{worker_id}.gpkg"
    clip_path = test_dir / f"GFA_V2_{run_tag}_w{worker_id}_clip.gpkg"
...
def merge_and_export(run_tag, n_workers, base_name=None):
    ...
    worker_paths = [test_dir / f"GFA_V2_{run_tag}_w{k}.gpkg"
    ...
    base     = base_name or f"GFA_V2_{run_tag}"
    ...
    clip_worker_paths = [
        test_dir / f"GFA_V2_{run_tag}_w{k}_clip.gpkg"
        for k in range(n_workers)]
...
def save_cartographic_layers(aoi_geom, dem_path, gdf_result, output_dir, base_name):
    """Recorte de capas de ejemplo al AOI. En GFA_V2 NO hay raster de origen que
...
def run_orchestrator():
    ...
    print(f"\n{'='*58}")
    print(f"  ORQUESTADOR — GFA_V2 (paralelo por anio)")
    print(f"  Anios   : {years}")
    ...
    partial = len(fail) > 0
    base_name = (f"GFA_V2_{run_tag}" + ("_PARCIAL" if partial else ""))
    if partial:
