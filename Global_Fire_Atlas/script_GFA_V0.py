...
"""
Version 0.0.0  (prefijo de salida "GFA_V0")
@author: jvilla
...
======================= Cambios GFA_V0 (respecto a BA_V11) =======================
...
[VER]    Prefijo de salida "V11" -> "GFA_V0".
...
REGIONS_PATH = MCD14ML_INPUT / "region-geografica.shp"
REGION_FIELD = "nombre"
...
COL_ORDER = [
    ...
    "region_geo", "cl12_m2", "cl3_m2", "ADM0_CODE", "gaul0_code",
...
RESERVED_GENERATED = {
    "event_uid", "year", "month", "dem_median", "region_geo",
    "cl12_m2", "cl3_m2", "ADM0_CODE", "gaul0_code", "gaul0_name", "area_ha",
    "index_right", "_match",
}
...
# --- DEM: mediana zonal por poligono (NUEVO en GFA_V0) -----------------------
...
def calc_class_clip(polygons_gdf, year, mb_meta, tiles_map, target_class):
    ...
    Semantica de area:
        ...
        >0  -> area real de la interseccion evento ∩ claseK, en m2 (EPSG:3857).
    """
    n = len(polygons_gdf)
    clip_geoms = [None] * n
...
def enrich_events(gdf, countries_gdf, regions_gdf,
                  mb_meta_c12, mb_meta_c3, year):
    """sjoin pais + COUNTRY_FILTER + region + area + recorte por clase.

    Recibe un GeoDataFrame de eventos (ya con event_uid/year/month/start_date/
    dem_median). Devuelve (gdf, gdf_clip).
    """
    if gdf is None or len(gdf) == 0:
        return None, None
    ...
    for region, target_class, tiles_map, mb_meta, col in plan:
        ...
        for pos, gi in enumerate(sub_idx):
...
def process_gfa_year(year, shp_path, countries_gdf, regions_gdf,
    ...
    gdf, gdf_clip = enrich_events(gdf0, countries_gdf, regions_gdf,
                                  mb_meta_c12, mb_meta_c3, year)
    if gdf is None or len(gdf) == 0:
...
def run_worker(worker_id, n_workers):
    ...
    print(f"\n{'='*58}\n  WORKER {worker_id}/{n_workers} — GFA_V0")
    ...
    gpkg_path = test_dir / f"GFA_V0_{run_tag}_w{worker_id}.gpkg"
    clip_path = test_dir / f"GFA_V0_{run_tag}_w{worker_id}_clip.gpkg"
...
def merge_and_export(run_tag, n_workers, base_name=None):
    ...
    worker_paths = [test_dir / f"GFA_V0_{run_tag}_w{k}.gpkg"
    ...
    base     = base_name or f"GFA_V0_{run_tag}"
    ...
    clip_worker_paths = [
        test_dir / f"GFA_V0_{run_tag}_w{k}_clip.gpkg"
        for k in range(n_workers)]
...
def save_cartographic_layers(aoi_geom, dem_path, gdf_result, output_dir, base_name):
    """Recorte de capas de ejemplo al AOI. En GFA_V0 NO hay raster de origen que
...
def run_orchestrator():
    ...
    print(f"\n{'='*58}")
    print(f"  ORQUESTADOR — GFA_V0 (paralelo por anio)")
    print(f"  Anios   : {years}")
    ...
    partial = len(fail) > 0
    base_name = (f"GFA_V0_{run_tag}" + ("_PARCIAL" if partial else ""))
    if partial:
    ...
    if ok:
        ...
        if gpkg_out is not None:
            ...
            cols_show = ["event_uid", "year", "month", "start_date",
                         "dem_median", "region_geo", "cl12_m2", "cl3_m2",
                         "area_ha", "gaul0_name"]
            try:
